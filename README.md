...
### Restarting an *etcd* node, changing IP

If an *etcd* node is restarted and changes IP, it should rejoin the cluster using `inital-cluster-state=existing` (see [etcd documentation](https://coreos.com/etcd/docs/latest/runtime-reconf-design.html)), but the `etcd.service` file generated by Ansible sets `inital-cluster-state=new` for the initial start. So the node is not able to join the existing cluster.

This event may happen if the instance is restarted by an external agent or reprovisioned by Terraform (and then reinstalled by Ansible).

The playbook should be smart enough to understand the cluster already exists, the peerURL was already registered, but with a different member ID. If it is the case, it should configure the node with `inital-cluster-state=existing`.

Alternatively, it should remove the old member with the same peerURL, then add the new one, using `etcdctl`. See documentation for [Runtime Configuration](https://coreos.com/etcd/docs/latest/runtime-configuration.html)
